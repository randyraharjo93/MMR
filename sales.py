...
class mmr_laporanmarketing(osv.osv):
    ...
    def _isi_penjualanpo(self):
        ...
        po_search_term = []
        faktur_search_term = []
        if self.customer or self.rayon or self.kota or self.starttanggal or self.endtanggal or self.teknisi:
            start_time = time.time()
            if self.customer:
                po_search_term.append(('customer', '=', self.customer.id))
                faktur_search_term.append(('customer', '=', self.customer.id))
            ...
            listrayonretur = []
            if self.rayon:
                po_search_term.append(('rayon', '=', self.rayon.id))
                faktur_search_term.append(('rayon', '=', self.rayon.id))
                for semuapenjualanretur in self.env['mmr.penjualanretur'].search([]):
            ...
            listkotaretur = []
            if self.kota:
                po_search_term.append(('kota', '=', self.kota.id))
                faktur_search_term.append(('kota', '=', self.kota.id))
                for semuapenjualanretur in self.env['mmr.penjualanretur'].search([]):
            ...
            if self.starttanggal:
                po_search_term.append(('tanggal','>=',self.starttanggal))
                faktur_search_term.append(('tanggalterbit','>=',self.starttanggal))
            ...
            listendtanggalretur = []
            if self.endtanggal:
                po_search_term.append(('tanggal','<=',self.endtanggal))
                faktur_search_term.append(('tanggalterbit','<=',self.endtanggal))
            ...
            listteknisiretur = []
            if self.teknisi:
                po_search_term.append(('teknisi','=',True))
                faktur_search_term.append(('teknisi','=',True))
            print '1--------------------------------------------------------'
            print("--- %s seconds ---" % (time.time() - start_time))
            print po_search_term
            print faktur_search_term
            start_time = time.time()
            listintersect = self.env['mmr.penjualanpo'].search(po_search_term)
            listintersectfaktur = self.env['mmr.penjualanfaktur'].search(faktur_search_term)
            print '2--------------------------------------------------------'
            print("--- %s seconds ---" % (time.time() - start_time))
            ...
            start_time = time.time()
            self.penjualanpo = [(6,0,listintersect.ids)]
            self.penjualanfaktur = [(6,0,listintersectfaktur.ids)]
            print '4--------------------------------------------------------'
            print("--- %s seconds ---" % (time.time() - start_time))
            # for semuaid in listintersect:
            #     self.penjualanpo += semuaid
                
            # for semuaid in listintersectfaktur:
            #     self.penjualanfaktur += listintersectfaktur
            start_time = time.time()
